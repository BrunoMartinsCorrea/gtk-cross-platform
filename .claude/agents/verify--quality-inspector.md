...
- Composition root: `src/app.rs::activate()` is the primary wiring point — `MainWindow` may also re-wire for runtime
  switching
...
Severity: **CRITICAL** (breaks functionality or safety) | **HIGH** (violates architecture) | **MEDIUM** (violates
convention) | **LOW** (style issue)
