...
> **Release pipeline — Step 2 of 3:** `release:artifact-packaging` → `release:artifact-signing` →
`release:store-submission`

Invoke with `/release:artifact-signing` after packaging is complete and before store submission.

## When to use

- After `/release:artifact-packaging` produces all platform artifacts
