...
Invoke with `/plan:scope-definition` to frame a feature before decomposing it.

## When to use

- When a user request is ambiguous ("add container filtering", "improve performance")
- Before `/plan:work-planning` to ensure the scope is unambiguous
...
Example: "Users cannot filter containers by status, forcing them to scroll through long lists to find running
containers."
...
A structured scope document with all 5 sections filled. This becomes the reference for `/plan:work-planning` and the PR
description.
