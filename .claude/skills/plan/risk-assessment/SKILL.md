...
Invoke with `/plan:risk-assessment` to surface risks before implementation starts.
...
**Risk:** All 5 adapter implementations (Docker, Podman, containerd, Mock, Dynamic) must implement the new method.
Missing implementations cause compile errors across all test targets.
