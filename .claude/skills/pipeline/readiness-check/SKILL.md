...
Invoke with `/pipeline:readiness-check` before writing the first line of implementation code.
...
- [ ] All acceptance criteria are concrete and testable (from `/pipeline:scope-definition`)

### Dependencies

- [ ] No circular dependencies in the planned task order (from `/pipeline:dependency-mapping`)
