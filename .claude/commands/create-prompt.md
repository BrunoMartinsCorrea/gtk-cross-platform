---
description: Generate a high-quality, self-contained AI prompt for a given context. Usage: /project:create-prompt <context-description>
---
...
**Usage:** `/project:create-prompt <context-description>`
...
|---|-----------|---------------|
| I1 | `$ARGUMENTS` is non-empty | If empty or whitespace-only, fail with: "Provide a context description. Example: `/project:create-prompt audit de segurança para APIs REST`" |
...
| Field | How to derive |
|-------|---------------|
| **Domain** | The subject area (e.g., "GTK4", "segurança de APIs", "análise de post-mortem") |
| **Goal** | What outcome the prompt should produce (e.g., "relatório de gaps", "código refatorado") |
...
- Print: "Prompt `<slug>.md` already exists (version `<version from frontmatter>`). Overwriting with version `<current_version + 0.1>`."
...
Apply the **RISEN + CO-STAR hybrid** framework. Use the Output Template (§Output Template) exactly — do not omit or reorder sections.

**Language rule:** every section heading and body text must be in the language determined in Step 1. Do not mix languages. If language = "pt", use the Portuguese heading names in the template; if language = "en", translate them to English equivalents.
...
Sort all entries alphabetically by title (case-insensitive). If the slug already existed, replace its existing line rather than adding a duplicate. Write the updated file.
...
## Critérios de Aceite de Entrada
...
## Critérios de Aceite de Saída

After completing the task, verify these criteria. If any fail, append a `## Erros de validação`
...
- ❌ Steps that describe state instead of actions ("The code should be clean" vs "Remove all `#[allow(dead_code)]` attributes")
