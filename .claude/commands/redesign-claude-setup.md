...
**Usage:** `/project:add-use-case <use-case-name>`  
**Example:** `/project:add-use-case list-running-containers`
...
**Usage:** `/project:audit-layer-boundaries`
...
**Usage:** `/project:add-ci-quality-gates`
...
**Usage:** `/project:prepare-release <version>`  
**Example:** `/project:prepare-release 0.2.0`
...
3. Update the Slash Commands table in `CLAUDE.md` to include all commands (existing + new):
   ...
   | `/project:implement-container-ui` | Implement or overhaul the GTK4/Adwaita UI layer |
   | `/project:scaffold-oss-docs` | Generate OSS documentation structure |
   | `/project:update-gitignore` | Audit `.gitignore` against detected stack |
   | `/project:redesign-claude-setup` | Audit and regenerate the Claude command set |
   | `/project:add-use-case <name>` | Add a domain use case with hexagonal wiring |
   | `/project:audit-layer-boundaries` | Detect and fix hexagonal boundary violations |
   | `/project:audit-i18n` | Audit i18n completeness in `src/window/` |
   | `/project:audit-a11y` | Audit accessibility compliance in `src/window/` |
   | `/project:add-ci-quality-gates` | Add security, coverage, and widget-test CI jobs |
   | `/project:prepare-release <version>` | Prepare CHANGELOG, bump version, create tag |
...
Auxiliary files to create (run /project:add-ci-quality-gates to generate deny.toml):
...
| GTK widget tests in CI          | `/project:add-ci-quality-gates`                          |
| Code coverage                   | `/project:add-ci-quality-gates`                          |
| Lint (clippy)                   | `make lint` / CI ci.yml                                  |
| Format (rustfmt + EditorConfig) | `make fmt` / CI ci.yml / editorconfig.yml                |
| Security audit (CVEs)           | `/project:add-ci-quality-gates`                          |
| Dependency license audit        | `/project:add-ci-quality-gates`                          |
| i18n completeness               | `/project:audit-i18n`                                    |
| A11Y compliance                 | `/project:audit-a11y`                                    |
| Architecture boundaries         | `/project:audit-layer-boundaries`                        |
| New domain use case             | `/project:add-use-case <name>`                           |
| New runtime adapter             | `/project:add-runtime-driver <name>`                     |
| Conventional Commits            | `CONTRIBUTING.md` (enforced by convention)               |
| CHANGELOG + version bump        | `/project:prepare-release <version>`                     |
| Signed release tag              | `/project:prepare-release <version>`                     |
| Flatpak distribution            | CI flatpak.yml (triggers on tag / push to main)          |
| OSS documentation               | `/project:scaffold-oss-docs`                             |
| README / CONTRIBUTING           | `/project:update-readme`                                 |
