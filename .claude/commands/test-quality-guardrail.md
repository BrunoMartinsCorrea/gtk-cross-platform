...
fn mask_password_suffix() { assert!(is_secret_env_key("POSTGRES_PASSWORD")); ... }
fn mask_password_lowercase() { assert!(is_secret_env_key("password")); ... }
fn mask_secret_substring() { assert!(is_secret_env_key("API_SECRET")); ... }
...
Para cada problema encontrado nas Fases 2 e 3, aplique a melhoria diretamente. Não interrompa para pedir confirmação
entre melhorias, a menos que uma mudança afete a API pública de um módulo de produção.

**Restrições obrigatórias:**

- Modifique apenas arquivos da camada de testes (módulos `#[cfg(test)]` inline e arquivos em `tests/`)
- Não remova testes existentes — refatore-os para usar as novas abstrações
- Respeite a regra de pureza de camadas: helpers de teste na camada de domínio não devem importar tipos de
  infraestrutura ou UI
- Não introduza dependências que conflitem com o modelo de async/threading do projeto (consulte `CLAUDE.md`)
- Helpers de teste devem ser declarados dentro de `#[cfg(test)]` e não exportar símbolos públicos visíveis fora
  do escopo de testes
- O módulo `tests/support/` (se criado) deve usar `#[cfg(test)]` ou ser declarado apenas sob `[dev-dependencies]`
- Todo teste que passava antes da refatoração deve continuar passando após (`make test` deve sair com código 0)

**Ordem de prioridade para aplicação:**

1. Corrigir testes vacuosos (Fase 2.1) — prioridade máxima, pois mascaram regressões
2. Tornar o mock fiel (Fase 2.1 — Non-Failing Fake) — adicionar verificação de existência e estado nas operações
   que hoje sempre retornam `Ok(())`
3. Criar módulo de suporte compartilhado `tests/support/` com fixtures e constantes (Fases 3.1–3.2)
4. Aplicar testes parametrizados onde indicado (Fases 3.5–3.6)
5. Renomear testes com prefixo `test_` redundante e corrigir nomenclatura sem condição (Fase 1.2)
6. Fragmentar o teste deus (Fase 2.2)
7. Mover testes de pureza de camada para o módulo correto (Fase 2.3)

Ao concluir todas as melhorias, execute `make test` e reporte o resultado final.

---

## Fase 5 — Laudo Técnico (Entrega Obrigatória)

Esta fase produz o documento de saída. Execute-a **após** as Fases 1–4. O laudo é a entrega principal do comando —
...
### Fase 4 — Resumo das Melhorias Aplicadas

| Melhoria | Padrão Aplicado | Testes Afetados | Status |
|----------|----------------|-----------------|--------|
| ... | Builder | N testes | Aplicado |
| ... | Parametrizado | N testes | Aplicado |
...
- Modificar código de produção (`src/` fora de `#[cfg(test)]`)
- Remover um teste sem substituí-lo por um equivalente ou mais forte
- Introduzir dependências que conflitem com as restrições de runtime assíncrono do projeto
- Fazer testes que exigem display ou recursos externos rodarem sem o isolamento adequado
- Introduzir helpers de teste que importem tipos de uma camada arquitetural superior à do teste assistido
- Marcar como corrigido um item da checklist antes de verificar `make test`
- Reportar como gap algo que já está corretamente implementado — foque apenas em problemas reais
- Omitir a Fase 5 — o laudo em `docs/test-quality-audit.md` é a entrega obrigatória do comando
