...
pub fn log_container_error(error: &ContainerError, logger: &AppLogger) {
    match error {
        ContainerError::PermissionDenied => logger.critical(&format!("{error}")),
        ContainerError::ParseError(_)    => logger.critical(&format!("{error}")),
        ContainerError::NotFound         => logger.info(&format!("{error}")),
        _                                => logger.warning(&format!("{error}")),
    }
}
```

**Rule:** always call `log_container_error()` at the call site (views, app.rs), not inside the
error constructor. Never call `AppLogger` directly for `ContainerError` variants.
